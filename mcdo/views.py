...
def home(request):
    if 'uid' in request.session:
        ...
        context = {
            'employee': emp,
            'is_active': is_active,
            'is_not_head': str(is_active)[0] != '1',
            'is_executive': str(is_active)[0] != '4',
            'mcdo': mcdo,
            'pending_approvals': pending_approvals
        }
        
        return render(request, 'mcdo/home.html', context)
    else:
        return redirect('/')
...
def takemehome(request):
    if 'uid' in request.session:
        if str(request.session['uid'])[0] == '4':
            ...
            return dispMap(request, Airport.objects.all().values())     
        else:
            return redirect('/')  
    else:
        # Since, Head is not using a 'is_active'/'uid' session key  
        return redirect('/')